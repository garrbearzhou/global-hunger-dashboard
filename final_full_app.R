...
wb_data <- read_csv("data/raw/world bank/world_bank_data.csv", show_col_types = FALSE) %>%
  select(country, NY.GDP.PCAP.CD, SP.DYN.LE00.IN, SP.POP.TOTL) %>%
  rename(
    Area = country,
...
pip_data <- read_csv("data/raw/world bank/pip 2.csv", show_col_types = FALSE) %>%
  select(country_name, headcount_ratio_international_povline, gini) %>%
  rename(
    Area = country_name,
    poverty_rate = headcount_ratio_international_povline,
...
combined_data <- fao_data %>%
  ...
  left_join(pip_data, by = "Area") %>%
  left_join(wfp_markets, by = "Area") %>%
  mutate(
